...
library(RTCGA.rnaseq) # gene expression
library(genefu)
library(graphite)
library(igraph)

setwd("WORKING DIRECTORY HERE") # set working directory
source('QualInt.R')
```
...
for(i in 1:(p-1)) {
  for(j in (i+1):p) {
    ...
    p.vals[i,j] <- RelDifLRT(theta.hat, standard.error, n, kappa = 2)$p.val
...
significant <- which(p.vals < .1/choose(p, 2), arr.ind = T)
adj <- apply(p.vals, c(1,2), function(x) ifelse(x < .1/choose(p, 2), 1, 0))
...
for(j in 1:p) {
  ...
  n <- c(n.plus, n.neg)
  kappa.surv[j] <- kappa.max(theta.hat,
                             standard.error,
                             n,
                             alpha = .1)
}
...
####################################
qual.ints <- which(kappa.surv > 1)
plus.dots <- which(kappa.surv > 1 & (abs(plus.hrs) > abs(neg.hrs)))
neg.dots <- which(kappa.surv > 1 & (abs(plus.hrs) < abs(neg.hrs)))
...
points(plus.hrs[plus.dots], neg.hrs[plus.dots],
       col = "orange", cex = 1.5, pch = 16)
points(plus.hrs[neg.dots], neg.hrs[neg.dots],
       col = "green", cex = 1.5, pch = 16)
...
tab <- data.frame(genes = KEGGgenes,
                  ...
                  neg.hrs = paste0(round(neg.hrs, 2), " (",
                                   round(neg.ses, 2), ")"),
                  kappa = kappa.surv)
tab <- tab[base::order(kappa.surv, decreasing = TRUE),]
colnames(tab) <- c("Gene", 
                   "ER+ Log HR (SE), SE", "ER- Log HR (SE)",
                   "$\\kappa^{\\alpha}_{\\max}$")
rownames(tab) <- NULL
tab <- tab[sort(kappa.surv,decreasing = TRUE) > 1, ]
print(xtable(tab),
